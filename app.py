...
with st.sidebar:
    ...
    st.markdown(f'<div class="sb-metric-value" style="font-size:1.05rem;">{meta["best_model_name"]}</div>', unsafe_allow_html=True)

    c1, c2 = st.columns(2)
    with c1:
        st.markdown('<div class="sb-metric-label">ROC-AUC</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="sb-metric-value">{meta["test_roc_auc"]:.3f}</div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="sb-metric-label">F1 Score</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="sb-metric-value">{meta["test_f1"]:.3f}</div>', unsafe_allow_html=True)
...
# Input form
# ---------------------------------------------------------------------------
st.markdown('<div class="section-head"><span class="dot"></span> Patient Information</div>', unsafe_allow_html=True)
...
with st.form("patient_form"):
    ...
    st.write("")
    submitted = st.form_submit_button("🔍  Predict Readmission Risk", width="stretch")
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                "threshold": {"line": {"color": INK, "width": 2}, "thickness": 0.8, "value": proba * 100},
            },
        )
    )
    fig.update_layout(
...
if submitted:
    ...
    st.write("")
    st.markdown('<div class="section-head"><span class="dot"></span> Prediction Result</div>', unsafe_allow_html=True)
    ...
    with r2:
        st.plotly_chart(make_gauge(proba), width="stretch", config={"displayModeBar": False})
    ...
    with r3:
        if proba >= 0.7:
            ...
            actions = ["Standard discharge process; no additional intervention indicated by the model."]
    ...
    # -----------------------------------------------------------------
    st.write("")
    st.markdown('<div class="section-head"><span class="dot"></span> Why This Prediction? (Explainable AI)</div>', unsafe_allow_html=True)
    ...
    with st.spinner("Computing explanation..."):
        ...
        try:
            ...
            colors = [ORANGE if v > 0 else TEAL for v in contrib.values]
